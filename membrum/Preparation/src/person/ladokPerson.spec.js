/**
 * Tests for the ladok parser.
 *
 * @date  2017-08-22
 * @since 8.4.0
 */

import { LadokPersonFromJSON, LadokPerson } from './ladokPerson'
...
describe('Ladok person.', () => {
  ...
  describe('join().', () => {
    ...
    it('Invalid.', () => {
      ...
      try {
        person.join(PERSON2)
        fail('Combining two people with different SSN' +
                    'should raise an error.')
      } catch (error) {
        expect(error.message).toMatch(`Can not combine non-equal ` +
                    `ladok people. SSNs:\n    ` +
                    `${person.ssn}\n    ${PERSON2.ssn}`)
      }
    })
  ...
  describe('samePerson()', () => {
    it('Equal people.', () => {
      ...
      expect(PERSON1.samePerson(
                new LadokPerson(PERSON1.ssn, 'A', 'B', 'C', 'D'))
            ).toBe(true)
    })
    it('Non-equal people.', () => {
      expect(PERSON1.samePerson(PERSON2)).toBe(false)
      expect(PERSON1.samePerson(
                new LadokPerson(PERSON2.ssn, 'A', 'B', 'C', 'D'))
            ).toBe(false)
    })
  })
  describe('toJSON()', () => {
