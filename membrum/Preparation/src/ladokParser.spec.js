/**
 * Tests for the ladok parseFiler.
 *
 * @date  2017-08-22
 * @since 7.9.0
 */

const { NumberMap } = require('common-js-utils')
const { getUnion, parseFile, parseDirectory, parseString, InvalidFile } =
  require('./ladokParser')
...
describe('Ladok parser.', () => {
  describe('Parse file.', () => {
    ...
    it('Incorrect file.', async () => {
      try {
        ...
        expect(error).toEqual(
          new InvalidFile('Invalid LADOK file, column line not ' +
    ...
    describe('All files.', () => {
      it('Points.', async () => {
        ...
        expect(people['9105040035'].points).toEqual(new NumberMap({
          'EHL': 35.5,
          'HT': 30.5,
          'JUR': 27.5,
          'KO': 137.5,
        ...
        expect(people['9006211537'].points).toEqual(new NumberMap({
          'EHL': 21.5,
          'HT': 2.5,
          'JUR': 12.5,
          'KO': 0.5,
    ...
    it('Read directory to object.', async () => {
      const people = await parseDirectory('mocks/ladok', true)
  ...
  it('Parse string.', async () => {
    const people = await parseString(
      ...
      '9006211537;ZIMON KUHS;eng08zku@student.lu.se; 21,5;', 'EHL')
  ...
  describe('Get union.', () => {
    ...
    it('Correct line, incorrect file.', () => {
      expect(getUnion('dummy.txt', [[
        'Lokal klass: EHL'
      ]])).toEqual('EHL')
    })
    it('Incorrect case.', () => {
      try {
        const union = getUnion('dummy.txt', [[
          'Lokal klazz: EHL'
        ]])
        fail(`Wrong file name and erroneous lines should raise an error ` +
          `in getUnion.`)
      } catch (error) {
        expect(error).toEqual(
          new InvalidFile(`Invalid LADOK file 'dummy.txt', ` +
            `could not determine union.`
          )
        )
      }
    })
