...
describe('membershipUpgradeTrail()', () => {
  let storeState;
  beforeEach(() => {
    storeState = createStateWithPlans();
  });
  it('should reassign trail to the given planId', () => {
    storeState = membership(storeState, membershipUpdatePlans(['1', '4']));
    storeState = membership(storeState, membershipUpgradeTrail('2'));
    expect(getSubscription(storeState)).toEqual(['1', '2']);
  });
  it('should return prevState if there is no trail to reassign', () => {
    storeState = membership(storeState, membershipUpdatePlans(['1', '3']));
    storeState = membership(storeState, membershipUpgradeTrail('2'));
    expect(getSubscription(storeState)).toEqual(['1', '3']);
  });
});
...
describe('isSubscriptionPaid()', () => {
  ...
  it.skip('should not be valid if a not paid dublicate plan is added', () => {
    storeState = membership(storeState, membershipUpdatePlans(['1', '2']));
    storeState = membership(storeState, membershipPay(['1', '2']));
    storeState = membership(storeState, membershipUpdatePlans(['1', '2', '2']));
    expect(isSubscriptionPaid(storeState, date)).toBeFalsy();
  });
});
...
describe('getUnpaidPlans()', () => {
  ...
  it('handles multiple payments during same interval', () => {
    ...
    storeState = membership(storeState, membershipPay(['1']));
    storeState = membership(storeState, membershipPay(['2']));
    storeState = membership(storeState, membershipUpdatePlans(['1', '2', '3']));
